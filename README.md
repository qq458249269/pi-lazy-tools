...
主 Agent 能调用哪些工具、每个工具几千字节的说明书，很大程度上决定了它能做什么。这些说明书每一轮请求都完整出现在上下文里，包括那些 95% 场合根本用不上的工具：主 Agent 不得不一遍遍重新扫过它们，注意力持续被稀释。注意力稀释的代价远大于多花的 token，计费只是它最表层的体现。我们此前做的 [async-subagent-isolation](https://github.com/Wolido/async-subagent-isolation) 在 subagent 维度落实了这条「上下文纯净」哲学：主智能体只负责派活、不碰任务细节；工具维度还剩一类问题：低频工具的定义每轮都在场（经济账：60 轮 × 3KB 就是 180KB 的进出流量，而它可能几天才用一次）。
...
本设计优化的是每轮请求的上下文构成：低频工具的定义每轮都在场，随会话变长、重试增多而复利积累（token 账单一并增长）。因此粒度必须到单个工具，同一个扩展里的工具可以部分隐藏，`lazy-tools.json` 里精确到工具名。
...
lazy 化的收益是让主 Agent 的上下文只保留真正会用到的工具，注意力不再被低频定义占用；省下的每轮定义 token 是附带收益。代价是每次使用多一轮 `load_tools` 往返和 `call_tool` 间接层。低频、参数简单的工具收益最大（远程代理、会话恢复类）；高频工具请移出名单回归常驻。
