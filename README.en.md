...
The main agent's system prompt and the tool definitions it can see decide what it can do: the noisier its context, the harder it is for it to focus on the actual work. We worked out this "clean context" philosophy at the subagent level with [async-subagent-isolation](https://github.com/Wolido/async-subagent-isolation); on the tool dimension one problem remains. A tool used in maybe 5% of sessions still pays for its definition (description, parameter JSON Schema, guidelines) on every single request, even when it actually gets used once every few days. 60 requests × 3 KB is 180 KB of traffic that buys nothing.

Skills solve the same problem with progressive disclosure: load only what is needed. pi-lazy-tools applies that idea to tools. Registration stays as-is (the `--tools` whitelist and pi runtime metadata are untouched); at session start the listed tools are removed from the LLM-visible active set, granular to individual tools, so a single extension can be partially hidden. When one is needed, `load_tools` injects its usage instructions as plain text and `call_tool` executes on its behalf; the actual execution is always the target extension's own `execute`. Because the tools field and the system prompt never change during a session, lazy loading never invalidates the cache, on any model (argument in [How it works](#how-it-works)).
...
This design optimizes per-request token composition: tool definitions are billed every round and the cost compounds as sessions lengthen and retries accumulate. Granularity therefore has to be per tool; tools of the same extension can be hidden selectively, named individually in `lazy-tools.json`.
...
Lazy-loading saves per-request definition tokens at the cost of one extra `load_tools` round-trip and the `call_tool` indirection per use. Low-frequency tools with simple parameters benefit most (remote proxies, session-resume tools); keep hot tools out of the list.
